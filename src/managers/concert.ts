import { getRepository, Any, Between, ILike } from "typeorm";
...
export const addConcerts = async (concerts: ConcertObject[]) => {
  ...
  await Promise.all(
    newConcerts.map(async (concert) => {
      ...
      concertObject.conductor_unknown = concert.conductors[0] === "*Kapellimestari ei tiedossa";
...
// Describe
// Get concerts count
export const getConcertsCount = async () => {
  const repo = getRepository(Concert);
...
export const searchConcertsByNames = async (
  ...
  if (symphony) {
    ...
    const symphonyResponse = await symphonyRepo.find({
      where: { name: ILike(`%${symphony}%`) },
      relations: ["performances", "performances.concert"],
    });

    symphonyConcerts = symphonyResponse.map((x) => x.performances.map((perf) => perf.concert)).flat();
  }
  ...
  const array = composerConcerts.concat(musicianConcerts).concat(conductorConcerts).concat(symphonyConcerts);
  ...
  const concertRepo = getRepository(Concert);
  const concertsResponse = await concertRepo.findAndCount({
    where: whereQuery,
    relations: [
      "conductors",
      "concert_tag",
  ...
  const filterSymphonies = (concerts: Concert[]) =>
    concerts.filter((concert) =>
      findStringArrayMatch(
        concert.performances
          .map((perf) => perf.symphony)
          .flat()
          .map((symph) => symph.name),
        symphony,
        true
      )
    );
  ...
  const results = pipe(
    ...
    filterSoloists,
    filterSymphonies
  )(concertsResponse[0]).flat() as Concert[];
  ...
  const mapped: Partial<Concert>[] = results.map((x) => ({
    ...
    date: x.date,
    conductor_unknown: x.conductor_unknown,
  }));

  const resultsCount = (conductor || soloist || composer) && chunkIndex === 0 ? mapped.length : concertsResponse[1];

  // Return max 100
  return { total: resultsCount, results: mapped.splice(0, chunk) };
};

export const getConcertsYearRange = async () => {
  const repo = getRepository(Concert);
  const res = await repo
    .createQueryBuilder("concert")
    .select("MIN(concert.date)", "min")
    .addSelect("MAX(concert.date)", "max")
    .getRawOne();
  return res;
};
