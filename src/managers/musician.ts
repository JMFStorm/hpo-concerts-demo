...
export const getComposersByKeyword = async (keyword: string) => {
  ...
  let whereArr: any[] = [{ name: ILike(`%${keyword}%`) }];
  const keywords: string[] = keyword.split(" ");

  if (keywords.length === 2) {
    whereArr = [];
    whereArr.push({ name: ILike(`%${keywords[0]}%${keywords[1]}%`) });
    whereArr.push({ name: ILike(`%${keywords[1]}%${keywords[0]}%`) });
  }

  const response = await repo.find({ where: whereArr, order: { name: "ASC" } });
  return response;
};
