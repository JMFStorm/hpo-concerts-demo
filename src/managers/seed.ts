...
export const parseSymphoniesFromRows = (rows: CsvRowObject[], premiereTags: PremiereTagConfig[]) => {
  ...
  rows.forEach((row) => {
    ...
    if (
      ...
      const message = `Id ${newSymphonyObj.symphony_id}: '${newSymphonyObj.name}' != '${previous?.name}'`;
...
export const parseConductorsFromRows = (rows: CsvRowObject[]) => {
  ...
  rows.forEach((x) => {
    ...
    } else if (conductor !== "" && !conductors.includes(conductor) && conductor !== "*Kapellimestari ei tiedossa") {
