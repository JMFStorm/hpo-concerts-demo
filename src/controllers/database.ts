...
const seedDatabase = async (csvRows: CsvRowObject[]) => {
  try {
    ...
    const logDeletion = () => seedLog(`Deleted ${++deletedEntities}/${totalEntities} entities.`);
    await Promise.all([
    ...
    seedLog(`Saved ${totalAdded}/${totalCount} performances`, "total");
    while (csvRows.length > 0) {
      ...
      const progressPercent = (totalAdded / totalCount) * 100;
      seedLog(`Saved ${totalAdded}/${totalCount} performances.`, "total", {
    ...
    seedLog(`${totalAdded} performances in total.`, "result", { completed: true });
    return totalAdded;
  } catch (err) {
...
// Describe
// Validate csv data for seed
controller.post("/validate", validateToken, async (req, res, next) => {
