...
export const fetchAllSymphoniesByKeyword = async (keyword) => {
  const url = `api/symphony/search?keyword=${keyword}`;
  const { result, error } = await get(url);
  return { result, error };
};

export const fetchSymphoniesByComposerName = async (composerName) => {
  const url = `api/symphony/composername/${composerName}`;
  const { result, error } = await get(url);
  return { result, error };
};
...
export const fetchConcertsCombinationSearch = async (
  ...
  let urlPath = `api/concert/combination/search?startyear=${startYear}&endyear=${endYear}`;
  ...
  if (symphony) {
    urlPath = urlPath.concat(`&symphony=${symphony.toLowerCase()}`);
  }
  if (chunkIndex) {
...
export const fetchConcertsTotalCount = async () => {
  const urlPath = "api/concert/search/total";
  const count = await get(urlPath);
  return count;
};

export const fetchConcertsYearRange = async () => {
  const urlPath = "api/concert/years/range";
  const count = await get(urlPath);
  return count;
};
...
export const fetchSymphonyById = async (symphonyId) => {
  const urlPath = `api/symphony/searchid/${symphonyId}`;
  const { result, error } = await get(urlPath);
  return { result, error };
};
...
export const validateCsvData = async (csvData, token) => {
  const urlPath = `api/database/validate`;
